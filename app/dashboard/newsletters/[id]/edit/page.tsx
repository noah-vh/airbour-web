...
export default function EditNewsletterPage() {
  ...
  // Queries
  const newsletter = useQuery(api.newsletters.get, { id: newsletterId });
  ...
  // Load newsletter data on mount
  useEffect(() => {
    if (newsletter && !isLoaded) {
      setNewsletterTitle(newsletter.title || "");
      setNewsletterSubject(newsletter.subject || "");
      if (newsletter.sections && Array.isArray(newsletter.sections)) {
        setSections(newsletter.sections.map((s: any, index: number) => ({
          id: s.id || `section-${index}`,
          type: s.type || "custom_content",
          title: s.title || "",
          content: s.content || "",
      ...
      if (newsletter.sourceSignalIds) {
        setSelectedSignalIds(newsletter.sourceSignalIds);
      }
      if (newsletter.sourceMentionIds) {
        setSelectedMentionIds(newsletter.sourceMentionIds);
      }
      setIsLoaded(true);
      setLastSaved(newsletter.updatedAt ? new Date(newsletter.updatedAt) : null);
    }
  }, [newsletter, isLoaded]);
  ...
  }, [newsletterId, newsletterTitle, newsletterSubject, sections, updateNewsletter]);

  // Debounced auto-save
  useEffect(() => {
    if (hasUnsavedChanges && isLoaded && newsletterTitle && newsletterSubject) {
  ...
  }, [hasUnsavedChanges, isLoaded, newsletterTitle, newsletterSubject, saveNewsletter]);

  // Mark as unsaved on changes (only after initial load)
  useEffect(() => {
    if (isLoaded) {
      setHasUnsavedChanges(true);
    }
  }, [newsletterTitle, newsletterSubject, sections, isLoaded]);
  ...
  return (
    <div className={cn(
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-[300px] border-r border overflow-y-auto p-4 space-y-4">
          {/* Newsletter Metadata */}
        ...
        <div className="w-[280px] border-l border overflow-y-auto p-4">
            ...
            {SECTION_TYPES.map((sectionType) => {
              ...
              return (
                <button
                  ...
                  className="w-full p-3 rounded-lg bg-muted border hover:bg-muted/80 hover:border/80 transition-colors text-left group"
                >
      ...
      {showSaveTemplateModal && (
        <div className="fixed inset-0 bg-black/60  z-50 flex items-center justify-center p-4">
          <div className="bg-background border rounded-xl w-full max-w-md">
            ...
            <div className="flex items-center justify-end gap-3 p-4 border-t border">
              <button
                onClick={() => setShowSaveTemplateModal(false)}
